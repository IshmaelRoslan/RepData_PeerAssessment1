...
ggplot(times, aes(x=interval, y=steps)) +
        geom_line() +
        xlab("Interval") +
        ylab("Steps") +
...
Replace missing values in **activity** with mean from the **times** table.
```{r missing}
for (i in 1:nrow(activity)) {
        if (is.na(activity[i, 1]) == TRUE)
        {
                activity[i, 1] <- times[match(activity[i, 3], times$interval), 2]
        }
}
```
